// generated with ast extension for cup
// version 0.8
// 21/0/2021 15:53:15


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(ConstParamComma ConstParamComma) { }
    public void visit(Mulop Mulop) { }
    public void visit(GlobalVarComma GlobalVarComma) { }
    public void visit(ExtendType ExtendType) { }
    public void visit(FormParList FormParList) { }
    public void visit(Relop Relop) { }
    public void visit(TermList TermList) { }
    public void visit(ClassVarDeclIf ClassVarDeclIf) { }
    public void visit(MethodType MethodType) { }
    public void visit(DecSingle DecSingle) { }
    public void visit(ConstParamSemi ConstParamSemi) { }
    public void visit(VarDeclIf VarDeclIf) { }
    public void visit(DesAssignIFErr DesAssignIFErr) { }
    public void visit(FormParParen FormParParen) { }
    public void visit(StatementList StatementList) { }
    public void visit(ClassVarDecl ClassVarDecl) { }
    public void visit(MethodDecList MethodDecList) { }
    public void visit(Addop Addop) { }
    public void visit(Factor Factor) { }
    public void visit(CondTerm CondTerm) { }
    public void visit(VarList VarList) { }
    public void visit(ConstList ConstList) { }
    public void visit(Designator Designator) { }
    public void visit(Term Term) { }
    public void visit(Condition Condition) { }
    public void visit(CaseParam CaseParam) { }
    public void visit(DecList DecList) { }
    public void visit(CaseList CaseList) { }
    public void visit(GlobalVarParamSemi GlobalVarParamSemi) { }
    public void visit(FormParListIf FormParListIf) { }
    public void visit(FormParComma FormParComma) { }
    public void visit(Expr Expr) { }
    public void visit(ActPars ActPars) { }
    public void visit(ConditionParen ConditionParen) { }
    public void visit(VarParam VarParam) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(Statement Statement) { }
    public void visit(ClassDecl ClassDecl) { }
    public void visit(ConstParam ConstParam) { }
    public void visit(CondFact CondFact) { }
    public void visit(FormPar FormPar) { }
    public void visit(GlobalVarList GlobalVarList) { }
    public void visit(MulMod MulMod) { visit(); }
    public void visit(MulDiv MulDiv) { visit(); }
    public void visit(MulMul MulMul) { visit(); }
    public void visit(AddMinus AddMinus) { visit(); }
    public void visit(AddPlus AddPlus) { visit(); }
    public void visit(ReLSE ReLSE) { visit(); }
    public void visit(RelLS RelLS) { visit(); }
    public void visit(RelBGE RelBGE) { visit(); }
    public void visit(RelBG RelBG) { visit(); }
    public void visit(RelDiff RelDiff) { visit(); }
    public void visit(RelEqual RelEqual) { visit(); }
    public void visit(Assignop Assignop) { visit(); }
    public void visit(DesignatorSingle DesignatorSingle) { visit(); }
    public void visit(DesArr DesArr) { visit(); }
    public void visit(DesMember DesMember) { visit(); }
    public void visit(FactorParen FactorParen) { visit(); }
    public void visit(FactorNewArr FactorNewArr) { visit(); }
    public void visit(FactorNew FactorNew) { visit(); }
    public void visit(FactorChar FactorChar) { visit(); }
    public void visit(FactorBool FactorBool) { visit(); }
    public void visit(FactorNum FactorNum) { visit(); }
    public void visit(FunCallParam FunCallParam) { visit(); }
    public void visit(FunCallNoParam FunCallNoParam) { visit(); }
    public void visit(FactVar FactVar) { visit(); }
    public void visit(FactorSingle FactorSingle) { visit(); }
    public void visit(FactorExpr FactorExpr) { visit(); }
    public void visit(TermSingleNeg TermSingleNeg) { visit(); }
    public void visit(TermSinglePos TermSinglePos) { visit(); }
    public void visit(TermExpr TermExpr) { visit(); }
    public void visit(TernDoTrue TernDoTrue) { visit(); }
    public void visit(TernCond TernCond) { visit(); }
    public void visit(ExprTern ExprTern) { visit(); }
    public void visit(ExprTermList ExprTermList) { visit(); }
    public void visit(CondFactOp CondFactOp) { visit(); }
    public void visit(CondFactNoOp CondFactNoOp) { visit(); }
    public void visit(CondTermSingle CondTermSingle) { visit(); }
    public void visit(CondTermList CondTermList) { visit(); }
    public void visit(ConditionSingle ConditionSingle) { visit(); }
    public void visit(ConditionList ConditionList) { visit(); }
    public void visit(ActParsSingle ActParsSingle) { visit(); }
    public void visit(ActParsList ActParsList) { visit(); }
    public void visit(ErrorDesignator ErrorDesignator) { visit(); }
    public void visit(DesAssignNoErr DesAssignNoErr) { visit(); }
    public void visit(DesStmDec DesStmDec) { visit(); }
    public void visit(DesStmInc DesStmInc) { visit(); }
    public void visit(DesStmActPar DesStmActPar) { visit(); }
    public void visit(DesStmNoActPar DesStmNoActPar) { visit(); }
    public void visit(DesStmAssign DesStmAssign) { visit(); }
    public void visit(CaseColonDef CaseColonDef) { visit(); }
    public void visit(CaseColon CaseColon) { visit(); }
    public void visit(CaseDefault CaseDefault) { visit(); }
    public void visit(CaseParamNum CaseParamNum) { visit(); }
    public void visit(CaseListEmpty CaseListEmpty) { visit(); }
    public void visit(CaseListParam CaseListParam) { visit(); }
    public void visit(ErrorCondition ErrorCondition) { visit(); }
    public void visit(ConditionPar ConditionPar) { visit(); }
    public void visit(SwitchStart SwitchStart) { visit(); }
    public void visit(DoStart DoStart) { visit(); }
    public void visit(IfOnlyStm IfOnlyStm) { visit(); }
    public void visit(ElseStm ElseStm) { visit(); }
    public void visit(ElseStart ElseStart) { visit(); }
    public void visit(WhileCondStart WhileCondStart) { visit(); }
    public void visit(SwitchExpr SwitchExpr) { visit(); }
    public void visit(StatementMult StatementMult) { visit(); }
    public void visit(StatementPrint StatementPrint) { visit(); }
    public void visit(StatementPrintLoop StatementPrintLoop) { visit(); }
    public void visit(StatementRead StatementRead) { visit(); }
    public void visit(StatementRetExpr StatementRetExpr) { visit(); }
    public void visit(StatementRet StatementRet) { visit(); }
    public void visit(StatementContinue StatementContinue) { visit(); }
    public void visit(StatementBreak StatementBreak) { visit(); }
    public void visit(StatementSwitch StatementSwitch) { visit(); }
    public void visit(StatementWhile StatementWhile) { visit(); }
    public void visit(StatementIfElse StatementIfElse) { visit(); }
    public void visit(StatementIf StatementIf) { visit(); }
    public void visit(StatementDesignator StatementDesignator) { visit(); }
    public void visit(NoStatementList NoStatementList) { visit(); }
    public void visit(MultStatement MultStatement) { visit(); }
    public void visit(FormParNoBrack FormParNoBrack) { visit(); }
    public void visit(FormParBrack FormParBrack) { visit(); }
    public void visit(ErrorFormParen ErrorFormParen) { visit(); }
    public void visit(FormParamNoError FormParamNoError) { visit(); }
    public void visit(ErrorFormComma ErrorFormComma) { visit(); }
    public void visit(FormParametarComma FormParametarComma) { visit(); }
    public void visit(FormParSingle FormParSingle) { visit(); }
    public void visit(FormParMult FormParMult) { visit(); }
    public void visit(NoFormParList NoFormParList) { visit(); }
    public void visit(FormParListUse FormParListUse) { visit(); }
    public void visit(MethType MethType) { visit(); }
    public void visit(MethVoid MethVoid) { visit(); }
    public void visit(MethodTypeName MethodTypeName) { visit(); }
    public void visit(MethodLocal MethodLocal) { visit(); }
    public void visit(MethodDecPar MethodDecPar) { visit(); }
    public void visit(NoExtendType NoExtendType) { visit(); }
    public void visit(ExtendTypeExist ExtendTypeExist) { visit(); }
    public void visit(ExtendErr ExtendErr) { visit(); }
    public void visit(ExtendNoError ExtendNoError) { visit(); }
    public void visit(NoMethodDecl NoMethodDecl) { visit(); }
    public void visit(MethodDeclUse MethodDeclUse) { visit(); }
    public void visit(ClassName ClassName) { visit(); }
    public void visit(ClassDeclMeth ClassDeclMeth) { visit(); }
    public void visit(ClassDeclNoMeth ClassDeclNoMeth) { visit(); }
    public void visit(ClassErrorVarSemi ClassErrorVarSemi) { visit(); }
    public void visit(ClassVarDeclNoErr ClassVarDeclNoErr) { visit(); }
    public void visit(NoClassVarDecl NoClassVarDecl) { visit(); }
    public void visit(ClassVarDeclUse ClassVarDeclUse) { visit(); }
    public void visit(NoVarDecl NoVarDecl) { visit(); }
    public void visit(VarDeclUse VarDeclUse) { visit(); }
    public void visit(VarParamNoBrack VarParamNoBrack) { visit(); }
    public void visit(VarParamBrack VarParamBrack) { visit(); }
    public void visit(SingleVar SingleVar) { visit(); }
    public void visit(MultVar MultVar) { visit(); }
    public void visit(VarDecl VarDecl) { visit(); }
    public void visit(GlobalErrorVarComma GlobalErrorVarComma) { visit(); }
    public void visit(GlobalCommaVar GlobalCommaVar) { visit(); }
    public void visit(GlobalErrorVarSemi GlobalErrorVarSemi) { visit(); }
    public void visit(GlobalVarSemi GlobalVarSemi) { visit(); }
    public void visit(GlobalSingleVar GlobalSingleVar) { visit(); }
    public void visit(GlobalMultVar GlobalMultVar) { visit(); }
    public void visit(GlobalVarDecl GlobalVarDecl) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(ConstChar ConstChar) { visit(); }
    public void visit(ConstBool ConstBool) { visit(); }
    public void visit(ConstNum ConstNum) { visit(); }
    public void visit(ConstErrorComma ConstErrorComma) { visit(); }
    public void visit(ConstCommaEnd ConstCommaEnd) { visit(); }
    public void visit(ConstName ConstName) { visit(); }
    public void visit(ConstErrorSemi ConstErrorSemi) { visit(); }
    public void visit(ConstSemi ConstSemi) { visit(); }
    public void visit(SingleConst SingleConst) { visit(); }
    public void visit(MultConst MultConst) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(DecClass DecClass) { visit(); }
    public void visit(DecVar DecVar) { visit(); }
    public void visit(DecConst DecConst) { visit(); }
    public void visit(NoDecl NoDecl) { visit(); }
    public void visit(MultDecl MultDecl) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
